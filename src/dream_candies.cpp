 ...
 */
bool set_customer_codes(const std::filesystem::path &customer_sample_path,
  ...
  customer_codes.erase(
      std::unique(customer_codes.begin(), customer_codes.end()),
      customer_codes.end());
...
/*!
 * \brief reads original customer file and creates new file containing only
 * records corresponding to the provided customer codes 
...
bool extract_customers(const std::vector<std::string> &customer_codes) {
  ...
  while (getline(original_customer_csv, line)) {
    ...
    if (getline(record, code, ',')) {
      if (binary_search(customer_codes.begin(), customer_codes.end(),
                        code.substr(1, code.length() - 2))) {
        extracted_customer_csv.write(line.c_str(), line.length());
        extracted_customer_csv.put('\n');
      }
    }
...
/*!
 * \brief reads original invoice item file and creates new file containing only
 * records corresponding to the provided invoice codes 
...
bool extract_invoice_items(std::vector<std::string> &invoice_codes) {
  ...
  invoice_codes.erase(std::unique(invoice_codes.begin(), invoice_codes.end()),
                      invoice_codes.end());
  ...
  while (getline(original_invoice_item_csv, line)) {
    std::stringstream record(line);
    if (getline(record, code, ',')) {
      if (binary_search(invoice_codes.begin(), invoice_codes.end(),
                        code.substr(1, code.length() - 2))) {
        extracted_invoice_item_csv.write(line.c_str(), line.length());
        extracted_invoice_item_csv.put('\n');
      }
...
/*!
 * \brief reads original invoice file and creates new file containing only
 * records corresponding to the provided customer codes. this function is also
 * responsible for collecting a vector of invoice codes to pass to
...
bool extract_invoices_and_items(const std::vector<std::string> &customer_codes) {
  ...
  while (getline(original_invoice_csv, line)) {
    ...
    if (getline(record, element, ',')) {
      if (binary_search(customer_codes.begin(), customer_codes.end(),
                        element.substr(1, element.length() - 2))) {
        extracted_invoice_csv.write(line.c_str(), line.length());
        extracted_invoice_csv.put('\n');
...
/*!
 * \brief creates new files containing only data related to the specified
 * customers if successful, extracted data files will be written to
 * extracted_files/ 
 * \param customer_sample_path path to a .csv containing customer codes 
...
bool extract_customer_data(const std::filesystem::path customer_sample_path) {
  ...
  if (set_customer_codes(customer_sample_path, customer_codes)) {
    if (extract_customers(customer_codes) &&
        extract_invoices_and_items(customer_codes)) {
      return true;
    }
  }
